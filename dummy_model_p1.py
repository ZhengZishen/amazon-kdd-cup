...
class vllm_Llama3ZeroShot(ShopBenchBaseModel):
    def __init__(self):
        random.seed(AICROWD_RUN_SEED)
        ...
        Question: Given the  event \"PersonX reads novels in a hammock\", as a result, PersonX\n0. is relaxed\n1. is caught by store owner\n2. buys a flashlight\n3. buys a wrench\nAnswer: \n
        0\n
        ...
        self.model = LLM(model="./models/Meta-Llama-3-8B-Instruct/", trust_remote_code=True, dtype='float16', tensor_parallel_size=2, enforce_eager=True)
...
class vllm_Llama3ZeroShot_v1(ShopBenchBaseModel):
    def __init__(self):
        random.seed(AICROWD_RUN_SEED)
        self.exampler = """
        [Question]: Select the category of products that can have the attribute closure type.\n0. sacco chair\n1. rangefinder\n2. cord management cover\n3. watch band\nAnswer: \n
        [Answer]: 3
        ...
        [Question]: "Extract phrases that indicate material composition from a customer query about a product. \nQuery: cotton blend shirt\nOutput: \n
        [Answer]: cotton blend
        
        [Question]: Which product category is an essential tool for a photographer?\n0. tripod\n1. notebook\n2. desk lamp\n3. bed sheet\nAnswer: \n
        [Answer]: 0
        
        """

        self.prompts = """
            You are a very intelligent and helpful online shopping assistant for Amazon who can give reasonable answers
             ([Answer]) after the online shopping questions ([Question]). 
            These are your Q&A:\n {}\nNow please continue to output / answer the rest part of the list until
            the answer or output of the last question is finished. If your answer is excellent, you will get tips.\n
            """.format(self.exampler)
        self.sampling_params = SamplingParams(temperature=0.1, top_p=0.95)
        self.model = LLM(model="./models/Qwen2.5-3B-Instruct/", trust_remote_code=True, dtype='float16',
